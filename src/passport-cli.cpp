...
#include "modules/validation/validation_security.hpp"
...
namespace {
	...
	std::string normalise_db_path(const std::string& raw) {
		...
		if (validation::is_directory(path)) {
		...
		std::string& master_pw)
	{
		const auto raw = terminal::prompt_for_input(
			"  Database path: ");
		if (validation::is_field_empty(raw)) {
		...
		if (validation::is_regular_file(db_path)) {
		...
		if (!validation_security::check_file_access(db_path, true)) {
			terminal::show_error(
		...
		if (validation::is_field_empty(master_pw)) {
			terminal::show_error("Master password cannot be empty.");
			return false;
		}
		if (!validation::is_master_password_length_valid(master_pw)) {
			terminal::show_error(
				"Master password must be between "
				+ std::to_string(validation::kPasswordMinLength_MasterPassword) + " and "
				+ std::to_string(validation::kPasswordMaxLength_MasterPassword) + " characters.");
			return false;
		}
		const auto confirm =
		...
		std::string& master_pw)
	{
		...
		if (!validation_security::check_file_access(db_path, false)) {
			terminal::show_error(
		...
		catch (const validation_security::AeadError&) {
			terminal::show_error(
				"Failed to open database. Wrong password or corrupted file.");
			return false;
		}
		catch (const validation_security::DeserialisationError& e) {
			terminal::show_error(
